...
const renumberMtgElements = () => {
    const mtgGroup = document.getElementById("mtgGroup");
    if (!mtgGroup) {
        return;
    }

    Array.from(mtgGroup.children).forEach((block, index) => {
        const number = index + 1;
        block.id = "mtgElement" + number;

        const minusButton = block.querySelector('.minusButton input');
        if (minusButton) {
            minusButton.id = "mtgMinusTime" + number;
        }

        const startSelect = block.querySelector('select[id^="MtgStartTime"]');
        if (startSelect) {
            startSelect.id = "MtgStartTime" + number;
        }

        const endSelect = block.querySelector('select[id^="MtgEndTime"]');
        if (endSelect) {
            endSelect.id = "MtgEndTime" + number;
        }

        const textInput = block.querySelector('input[id^="MtgText"]');
        if (textInput) {
            textInput.id = "MtgText" + number;
        }
    });
}
